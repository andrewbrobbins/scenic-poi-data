 ...
 * Usage: node build-fuel-ca-filter-brands.mjs
 */
import fs from "fs";
...
export function filterFuelCaBrands() {
  if (!fs.existsSync(ALL_FUEL_CACHE_PATH)) {
    throw new Error(`Missing cache ${ALL_FUEL_CACHE_PATH} — run: node build-fuel-ca-extract-all-pbf.mjs`);
  }
  ...
  for (const extracted of cache.records || []) {
    ...
    seen.add(key);
    const state = inferRegionCode(extracted.lat, extracted.lon, "CA") || "";
...
if (process.argv[1]?.endsWith("build-fuel-ca-filter-brands.mjs")) {
  filterFuelCaBrands();
}