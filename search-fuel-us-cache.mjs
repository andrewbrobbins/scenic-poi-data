/**
 * Search the cached US fuel extract by substring (fast brand iteration).
 * Usage: node search-fuel-us-cache.mjs "loves travel" [--limit=20]
 */
import { ALL_FUEL_CACHE_PATH, readJson } from "./fuel-us-lib.mjs";

const query = process.argv[2];
if (!query) {
  console.error('Usage: node search-fuel-us-cache.mjs "search terms" [--limit=50]');
  process.exit(1);
}
...
if (!cache?.records?.length) {
  ...
  console.error("Run: node build-fuel-us-extract-all-pbf.mjs");
  process.exit(1);
}
